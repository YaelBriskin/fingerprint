...
void fingerPrint()
{
  ...
  case '1':
    ...
    {
      lcd16x2_i2c_puts(0, 0, "No matching in the library"); // show on LCD
    ...
    else
    {
      ...
      if (id > 0 && result)
      {
        if (DB_write(id, timestamp, IN, FALSE) == SUCCESS) // write to database
        {
          ...
          lcd16x2_i2c_puts(0, 0, mydata); // show on LCD
          LOG_MESSAGE(LOG_DEBUG, __func__, "stderr", mydata, NULL);
          buzzer(); // Activate the buzzer for successful entry
        }
        else
        {
          lcd16x2_i2c_puts(0, 0, "Failed to write to database"); // show on LCD
  ...
  case '#':
    ...
    if (checkNewEmployeeSequence(input))
    {
      ...
      if (ack == 1)
      {
        ...
        lcd16x2_i2c_puts(0, 0, messageString); // show on LCD
        ...
        if (retries == g_max_retries)
        {
          ...
          deleteModel(id);
          DB_delete(id);
          lcd16x2_i2c_puts(0, 0, " Connection error. The new employee ID were not saved"); // show on LCD
      ...
      else
      {
        ...
        lcd16x2_i2c_puts(0, 0, "Enrolling failed."); // show on LCD
